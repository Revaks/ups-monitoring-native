...
При смене `SNMP_EXPORTER_PORT` **обязательно** поправьте `prometheus.yml`
(`replacement: localhost:<новый порт>`) — `run.sh` предупредит о расхождении
при старте. Затем `sudo systemctl restart ups-monitoring`.
